...
void Image::copyAndConvertDataFromImage(const Image& Inp) // функция преобразования данных переданного изображения в текущий формат
{
	...
	for (int i = 0; i < (int)BMInfoHeader.Height; i++)
		...
		{
			unsigned char grayscale = isSourceWithPalette ? Inp.Rgbtriple[i][j].Red // для исходного изображения с палитрой берем оттенок серого сразу из данных картинки
				: getGrayscaleColor(Inp.Rgbtriple[i][j].Red, Inp.Rgbtriple[i][j].Green, Inp.Rgbtriple[i][j].Blue); // иначе вычисляем его
...
Image& Image::operator /= (const Image& Inp) // Перегрузка оператора /=, выполняет запись переданного изображения в текущее с изменением размера
{
	...
	for (int i = 0; i < (int)BMInfoHeader.Height; i++)
		for (int j = 0; j < (int)BMInfoHeader.Width; j++)
		{
			...
			Rgbtriple[i][j] = Inp.Rgbtriple[sourceY][sourceX]; // запись значения цвета из исходного изображения в текущее
		}
...
void Image::saveImageDataToFile(FILE* f) // сохраняет данные в переданный файл
{
	...
	else // изображение с палитрой
	{
		...
		for (int i = BMInfoHeader.Height - 1; i >= 0; i--)
		{
			...
			for (int j = 0; j < (int)BMInfoHeader.Width; j++)
			{
				unsigned char paletteColorIndex = getNearestPaletteColorIndex(Rgbtriple[i][j].Red); // получаем индекс палитры для текущего цвета
...
{
	return BitCount == 24 || BitCount == 8 || BitCount == 4 || BitCount == 1; // 2-ой вариант
}
...
Image::Image(char Mode, WORD BCount, DWORD Width, DWORD Height)// Реализация конструктора создания изображения(с указанием параметров изображения)
{
	...
	// Поддерживается хранение данных только в RGBTRIPLE(вариант 2)
...
int main(void)
{
	...
	Image* im1 = new Image((char*)"van_gog_32.bmp"); // Создание объекта изображения из файла 
	Image* im2 = new Image(0, 24, 404*3/2, 512/2);// Создание изображения с заданными параметрами 
