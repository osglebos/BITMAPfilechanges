...
{
	// Проверяем совпадение разрешения, после чего скопируем данные
	...
	for (int i = 0; i < (int)BMInfoHeader.Height; i++)
		...
		{
			BYTE grayscale = isSourceWithPalette ? Inp.Rgbtriple[i * BMInfoHeader.Width + j].Red // для исходного изображения с палитрой берем оттенок серого сразу из данных картинки
				: getGrayscaleColor(Inp.Rgbtriple[i * BMInfoHeader.Width + j].Red, Inp.Rgbtriple[i * BMInfoHeader.Width + j].Green, Inp.Rgbtriple[i * BMInfoHeader.Width + j].Blue); // иначе вычисляем его
...
void Image::copyDataFromImage(const Image& Inp) // копирует данные при совпадении заголовков изображений
{
	...
	{
		Rgbtriple = new RGBTRIPLE[Inp.BMInfoHeader.Height*Inp.BMInfoHeader.Width];
		for (int i = 0; i < (int)Inp.BMInfoHeader.Height; i++)
			for (int j = 0; j < (int)Inp.BMInfoHeader.Width; j++)
				Rgbtriple[i*Inp.BMInfoHeader.Width + j] = Inp.Rgbtriple[i*Inp.BMInfoHeader.Width + j];
	}
	...
	else printf("Ошибка: в изображение уже проинициализированно другим разрешением.\n");
}
...
void Image::saveImageDataToFile(FILE* f) // сохраняет данные в переданный файл
{
	...
	if (BMInfoHeader.BitCount == 24)
	{
		...
		{
			for (int j = 0; j < (int)BMInfoHeader.Width; j++)
				fwrite(&(Rgbtriple[i*BMInfoHeader.Width + j]), sizeof(RGBTRIPLE), 1, f);
	...
	else // изображение с палитрой
	{
		...
		for (int i = BMInfoHeader.Height - 1; i >= 0; i--)
		{
			...
			for (int j = 0; j < (int)BMInfoHeader.Width; j++)
			{
				BYTE paletteColorIndex = getNearestPaletteColorIndex(Rgbtriple[i*BMInfoHeader.Width + j].Red); // получаем индекс палитры для текущего цвета
	...
	if (FileBMInfoHeader.BitCount == 24)
	{
		for (int i = BMInfoHeader.Height - 1; i >= 0; i--)
		{
			for (int j = 0; j < (int)BMInfoHeader.Width; j++)
			{
				...
				Rgbtriple[i * BMInfoHeader.Width + j].Red = filetriple.Red;
				Rgbtriple[i * BMInfoHeader.Width + j].Green = filetriple.Green;
				Rgbtriple[i * BMInfoHeader.Width + j].Blue = filetriple.Blue;
			}
		}
	}
...
void Image::setHeaderAndAllocateMemory(DWORD Width, DWORD Height, WORD BitCount)
{
	...
	BMInfoHeader.BitCount = BitCount; // Глубина цвета, число бит на точку = 1,4,8,24
...
{
	return BitCount == 24 || BitCount == 8 || BitCount == 4 || BitCount == 1; // 1-ый вариант
}
...
Image::Image(char Mode, WORD BCount, DWORD Width, DWORD Height)// Реализация конструктора создания изображения(с указанием параметров изображения)
{
	...
	// Поддерживается хранение данных только в RGBTRIPLE(вариант 1)
	...
	else printf("Ошибка: поддерживается создание изображения только 1, 4, 8, 24 бит.\n");// Выводим сообщение об ошибке
}
...
int main(void)
{
	...
	Image* im1 = new Image((char*)"tom_and_jerry_32.bmp"); // Создание объекта изображения из файла 
	Image* im2 = new Image(0, 24, 906/ 2, 497 * 6/ 7);// Создание изображения с заданными параметрами 
	Image* im3 = new Image(*im1);//// Создание объекта изображения из существующего изображения
	Image im4, im5;// Создание пустого изображения 

	(*im2) /= *im1; // Приведение img к масштабу img2 
	im4 = *im1 / 8; // делаем битность 8
	im5 = *im1 / 1; // делаем битность 1
	im2->writeimage((char*)"tom_and_jerry_res.bmp");
	im3->writeimage((char*)"tom_and_jerry_24.bmp");
	im4.writeimage((char*)"tom_and_jerry_8.bmp");
	im5.writeimage((char*)"tom_and_jerry_1.bmp");

	printf("tom_and_jerry.bmp have been converted.\n");
	scanf(" ");
	return 0;
}