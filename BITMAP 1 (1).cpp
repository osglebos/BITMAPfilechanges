...
{
	return BitCount == 24 || BitCount == 8 || BitCount == 4 || BitCount == 1; // 5-ый вариант
}
...
Image::Image(char Mode, WORD BCount, DWORD Width, DWORD Height)// Реализация конструктора создания изображения(с указанием параметров изображения)
{
	...
	// Поддерживается хранение данных только в RGBTRIPLE(вариант 5)
...
int main(void)
{
	...
	Image* im1 = new Image((char*)"coffee_32.bmp"); // Создание объекта изображения из файла 
	Image* im2 = new Image(0, 24, 1622 / 2, 1080 * 6 / 7);// Создание изображения с заданными параметрами 
	...
	im5 = *im1 / 4; // делаем битность 4
	im2->writeimage((char*)"coffee_res.bmp");
	im3->writeimage((char*)"coffee_24.bmp");
	im4.writeimage((char*)"coffee_8.bmp");
	im5.writeimage((char*)"coffee_4.bmp");

	printf("coffee.bmp have been converted.\n");
	scanf(" ");
	return 0;
}