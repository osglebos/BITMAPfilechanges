...
void Image::copyAndConvertDataFromImage(const Image& Inp) // функция преобразования данных переданного изображения в текущий формат
{
	...
	for (int i = 0; i < (int)BMInfoHeader.Height; i++)
		...
		{
			unsigned char grayscale = isSourceWithPalette ? Inp.Rgbquad[i][j].Red // для исходного изображения с палитрой берем оттенок серого сразу из данных картинки
...
int Image::loadImageDataFromFile(FILE* f)
{
	...
	if (FileBMInfoHeader.BitCount == 32)
	{
		for (int i = BMInfoHeader.Height - 1; i >= 0; i--)
		{
			for (int j = 0; j < (int)BMInfoHeader.Width; j++)
...
{
	return BitCount == 32 || BitCount == 8 || BitCount == 4 || BitCount == 1; // 2-ой вариант
}
...
int main(void)
{
	...
	Image* im1 = new Image((char*)"krolick.bmp"); // Создание объекта изображения из файла 
	Image* im2 = new Image(0, 32, 960, 270);// Создание изображения с заданными параметрами 
	...
	im3.writeimage((char*)"krolick_8.bmp");
	im4.writeimage((char*)"krolick_4.bmp");
	im2->writeimage((char*)"krolick_res.bmp");

	printf("krolick.bmp have been converted.\n");
	scanf(" ");
	return 0;
}
