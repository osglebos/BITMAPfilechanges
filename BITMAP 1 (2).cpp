...
void Image::copyAndConvertDataFromImage(const Image& Inp) // функция преобразования данных переданного изображения в текущий формат
{
	...
	for (int i = 0; i < (int)BMInfoHeader.Height; i++)
		...
		{
			BYTE grayscale = isSourceWithPalette ? Inp.Rgbquad[i][j].Red // для исходного изображения с палитрой берем оттенок серого сразу из данных картинки
				: getGrayscaleColor(Inp.Rgbquad[i][j].Red, Inp.Rgbquad[i][j].Green, Inp.Rgbquad[i][j].Blue); // иначе вычисляем его
			...
			Rgbquad[i][j].Red = grayscale;
			Rgbquad[i][j].Green = grayscale;
			Rgbquad[i][j].Blue = grayscale;
			Rgbquad[i][j].Reserved = grayscale;
		}
}
...
Image& Image::operator /= (const Image& Inp) // Перегрузка оператора /=, выполняет запись переданного изображения в текущее с изменением размера
{
	...
	for (int i = 0; i < (int)BMInfoHeader.Height; i++)
		for (int j = 0; j < (int)BMInfoHeader.Width; j++)
		{
			...
			Rgbquad[i][j] = Inp.Rgbquad[sourceY][sourceX]; // запись значения цвета из исходного изображения в текущее
		}
...
void Image::copyDataFromImage(const Image& Inp) // копирует данные при совпадении заголовков изображений
{
	...
	{
		Rgbquad = new RGBQUAD*[Inp.BMInfoHeader.Height];
		for (int i = 0; i < (int)Inp.BMInfoHeader.Height; i++)
		{
			Rgbquad[i] = new RGBQUAD[Inp.BMInfoHeader.Width];
...
void Image::saveImageDataToFile(FILE* f) // сохраняет данные в переданный файл
{
	...
	if (BMInfoHeader.BitCount == 24)
	{
		for (int i = BMInfoHeader.Height - 1; i >= 0; i--)
		{
			for (int j = 0; j < (int)BMInfoHeader.Width; j++)
			{
				...
				fileTriple.Blue = Rgbquad[i][j].Blue;
				fileTriple.Green = Rgbquad[i][j].Green;
				fileTriple.Red = Rgbquad[i][ j].Red;
	...
	else if (BMInfoHeader.BitCount == 32)
	{
		...
		{
			for (int j = 0; j < (int)BMInfoHeader.Width; j++)
				fwrite(&(Rgbquad[i][j]), sizeof(RGBQUAD), 1, f);
	...
	else // изображение с палитрой
	{
		...
		for (int i = BMInfoHeader.Height - 1; i >= 0; i--)
		{
			...
			for (int j = 0; j < (int)BMInfoHeader.Width; j++)
			{
				unsigned char paletteColorIndex = getNearestPaletteColorIndex(Rgbquad[i][j].Red); // получаем индекс палитры для текущего цвета
	...
	else if (FileBMInfoHeader.BitCount == 32)
	{
		for (int i = BMInfoHeader.Height - 1; i >= 0; i--)
		{
			for (int j = 0; j < (int)BMInfoHeader.Width; j++)
			{
				...
				Rgbquad[i][j].Red = fileQuad.Red;
				Rgbquad[i][j].Green = fileQuad.Green;
				Rgbquad[i][j].Blue = fileQuad.Blue;
				Rgbquad[i][j].Reserved = fileQuad.Reserved;
			}
		}
	}
...
{
	return BitCount == 32 || BitCount == 8 || BitCount == 4; // 7-ой вариант
}
...
Image::Image(char Mode, WORD BCount, DWORD Width, DWORD Height)// Реализация конструктора создания изображения(с указанием параметров изображения)
{
	...
	// Поддерживается хранение данных только в RGBQUAD(вариант 8)
...
int main(void)
{
	...
	Image* im1 = new Image((char*)"sky.bmp"); // Создание объекта изображения из файла 
	Image* im2 = new Image(0, 32, 1920*2, 1080*2/3);// Создание изображения с заданными параметрами 
	...
	im3.writeimage((char*)"sky_8.bmp");
	im4.writeimage((char*)"sky_4.bmp");
	im2->writeimage((char*)"sky_res.bmp");

	printf("sky.bmp have been converted.\n");
	scanf(" ");
	return 0;
}